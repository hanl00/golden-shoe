...
const appReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case actionTypes.TOGGLE_ITEM_IN_WISHLIST:
      let wishListTemp = state.wishlist;
      let chkProductInWishList = state.wishlist.find(
        (id) => id === action.productId
      );
      if (chkProductInWishList) {
        // remove from wish list
        wishListTemp = state.wishlist.filter((id) => id !== action.productId);
      } else {
        // add to wish list
        wishListTemp = state.wishlist.concat(action.productId);
      }
